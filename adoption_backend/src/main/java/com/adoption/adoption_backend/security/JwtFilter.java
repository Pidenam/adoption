        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
            ...
            try {
                if (jwtUtil.validerToken(token)) {
                    ...
                    List<String> permissions = jwtUtil.extrairePermissions(token);
                    ...
                    if (role.equals("PRESIDENT")) {
                        autorites.add(new SimpleGrantedAuthority("ALL_PERMISSIONS"));
                    } else {
                        for (String permission : permissions) {
                            autorites.add(new SimpleGrantedAuthority(permission));
                        }
                    }

                    UsernamePasswordAuthenticationToken auth =
                            new UsernamePasswordAuthenticationToken(email, null, autorites);
